...
function addOrUpdateJidState(jid, etat) {
  ...
  data[jid] = data[jid] || {};
  data[jid].etat = etat;
  data[jid].action = data[jid].action || 'remove'; // Default action is 'supp' if not present
...
function getJidAction(jid) {
  const data = readDataFromFile();
  return data[jid]?.action || 'remove'; // Default action is 'supp' if not present
}
...
module.exports = {
  ...
  // so those existing require() calls resolve correctly instead of getting
  // undefined for these names.
