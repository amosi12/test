...
bmbtz({
    ...
    try {
        checkedUsers = await Promise.all(
            numberList.map(async (v) => {
                const exists = await zk.onWhatsApp(v + '@s.whatsapp.net');
    ...
    const users = checkedUsers.filter((v) => v.exists).map((v) => v.num + '@s.whatsapp.net');
    ...
    let inviteCode;
    try {
        inviteCode = await zk.groupInviteCode(dest);
    } catch {
        inviteCode = null;
    }
